...
function _M:get_app_access_token()
    local url = "https://open.feishu.cn/open-apis/auth/v3/app_access_token/internal"
    local body = {
        app_id = self.app_id,
...
function _M:verify_token()
    ...
    if result["valid"] then
        ...
        if payload["name"] and payload["open_id"] then
            return payload
        end
...
function _M:sign_token(login_user)
    local open_id = login_user["open_id"]
    if not open_id or open_id == "" then
        return nil, "invalid open_id"
    end
    local name = login_user["name"]
...
function _M:check_user_access(user)
    ngx.log(ngx.ERR, "user name: ", user["name"], ", open_id: ", user["open_id"])
    ...
    if type(self.openid_whitelist) ~= "table" then
        ngx.log(ngx.ERR, "openid_whitelist is not a table")
        return false
    end
    -- ngx.log(ngx.ERR, "openid_whitelist", json.encode(self.openid_whitelist))
    if #self.openid_whitelist == 0 then
        return true
    end
    ...
    if has_value(self.openid_whitelist, open_id) then
        return true
    end
    -- local department_ids = user["department_ids"]
...
function _M:sso_callback()
    ...
    ngx.log(ngx.ERR, "callback login user name: ", login_user["name"], ", open_id: ", login_user["open_id"])
    ...
    if not self:check_user_access(login_user) then
        ngx.log(ngx.ERR, "user access not permitted")
        return self:sso()
    end

    local token, err = self:sign_token(login_user)
    if err ~= nil then
